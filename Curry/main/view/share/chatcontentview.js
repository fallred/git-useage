define([
	...
	'text!tpl/share/chatcontenttpl.html',
	'global'
	], function(Backbone,ChatCollection,tpl,global) {
	var self,flag;
	var ChatContentItemView = Backbone.View.extend({
		tagName: 'div',
	...
	var ChatContentView = Backbone.View.extend({
		...
		renderItem: function(item){
			var chatContentItemView = new ChatContentItemView({
				model:item
			});
			//解析信息表情，将UTF表情字符替换为图片
			var rendercontent = chatContentItemView.render().$el;
		...
		fun:function(type,message,date,senderid,receiverid,sendernickname,senderphoto){
		    self.collection.add({
		    	usernickname: global.currentChat.nickname,
		    	messagecontent:message,
		    	senderid:senderid,
		    	receiverid:receiverid,
		    	userphoto:global.currentChat.photo
		    });
		},
		scrollBtm: function(){
			if ( flag == true ){
